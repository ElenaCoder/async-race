...
function getNavigationMenu() {
  ...
  const btnToGarage = document.createElement('div');
  ...
  const btnToWinners = document.createElement('div');
  btnToWinners.className = 'to-winners button';
  const linkToWinners = document.createElement('a');
...
function getGarageViewContent() {
  ...
  const divGarageViewContent = document.createElement('div');
...
const garageViewContent = getGarageViewContent();
...
function getFormCreateFragment() {
  ...
  const inputTextFormCreate = document.createElement('input');
  ...
  const inputColorFormCreate = document.createElement('input');
...
const formCreateFragment = getFormCreateFragment();
...
function getFormUpdateFragment() {
  ...
  const inputTextFormUpdate = document.createElement('input');
  ...
  const inputColorFormUpdate = document.createElement('input');
...
async function showWinnerResultInGarage(winnerId, winnerTime) {
  const trackBlock = document.getElementById(winnerId);
  trackBlock.style.backgroundColor = 'var(--winner-color)';

  const spanWinnerMessage = document.createElement('span');
...
async function showWinnerResultInTable(winnerId, winnerTime) {
  const isCurrentWinnerInWinnerTable = await isInWinnersTable(winnerId);
  if (isCurrentWinnerInWinnerTable) {
    const response = await ServerRequest.getWinner(winnerId);
    const oldWins = await response.wins;
    const oldBestTime = await response.time;
    const newWins = +oldWins + 1;
    const newBestTime = winnerTime < oldBestTime ? winnerTime : oldBestTime;
...
async function resetInfoAfterRace() {
  const allInfoSpans = [...document.getElementsByClassName('race-info')];
...
async function stopCarEngineHandler(event) {
  const carIconElem = event.currentTarget.parentNode.nextElementSibling.firstElementChild;
  const carId = event.currentTarget.parentNode.parentNode.getAttribute('id');
  const currentStartBtn = event.currentTarget.parentNode.firstElementChild;
  const currentStopBtn = event.currentTarget.parentNode.lastElementChild;
  ...
  const carCurrentAnimation = carIconElem.getAnimations();
...
async function startCarEngineHandler(event) {
  ...
  const response = await ServerRequest.startCarEngine(carId);
  const time = response.distance / response.velocity;

  const carAnimation = carIconElem.animate(
    [
      { transform: 'translateX(0)' },
      { transform: 'translateX(82vw)' },
    ],
  ...
  try {
    ...
    if (currentWinnersArr.length === 1) {
      const winnerId = currentWinnersArr[0].id;
      const winnerTime = currentWinnersArr[0].time;
      if (raceStartedFlag) {
...
function createCarIcon(color) {
  const carIconFragment = document.createDocumentFragment();
  ...
  const iconpPath1 = document.createElementNS(
  ...
  const iconpPath2 = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path',
  );
  ...
  const iconpPath3 = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path',
  );
  ...
  const iconpPath4 = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path',
  );
  ...
  const iconpPath5 = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path',
  );
...
function createNavigationInCarBlockFragment(elem, node) {
  const divBlockNavigation = document.createElement('div');
  ...
  const linkSelect = document.createElement('a');
  ...
  const divBlockControlPanel = document.createElement('div');
...
function createCarTrack(elem, node) {
  const divBlockCarTrack = document.createElement('div');
  divBlockCarTrack.className = 'car-track';
  node.append(divBlockCarTrack);

  const carIcon = createCarIcon(elem.color);
...
function createCarBlockFragment(elem) {
  ...
  const divCarBlock = document.createElement('div');
...
async function renderCarsInGarage() {
  const carsInGarageResponse = await ServerRequest.getCars(
    currentPageGarage,
    carsNumberPerPageGarage,
  );
  const carsInGarageArr = await carsInGarageResponse.data;
  ...
  document.getElementsByClassName('cars-in-garage')[0].innerHTML = carsInGarageResponse.totalCount;
  ...
  const trackBlockList = document.getElementsByClassName('track-block');
  while (trackBlockList.length > 0) {
    trackBlockList[0].parentNode.removeChild(trackBlockList[0]);
  }

  const fragmentCarsBlock = document.createDocumentFragment();

  carsInGarageArr.forEach((elem) => {
    const fragmentCarBlock = createCarBlockFragment(elem);
...
async function generateCarHandler() {
  ...
  for (let i = 0; i < 100; i += 1) {
    const randomNamePart1 = randomNamePart1Arr[getRandomNumber(0, 10)];
    const randomNamePart2 = randomNamePart2Arr[getRandomNumber(0, 10)];
...
const formUpdateFragment = getFormUpdateFragment();
...
function getTaskbarFragment() {
  ...
  const divTaskbar = document.createElement('div');
  divTaskbar.className = 'taskbar';
  fragmentTaskbar.append(divTaskbar);

  const btnRace = document.createElement('div');
  btnRace.className = 'button race gray';
  const linkRace = document.createElement('a');
  ...
  const btnGenerateCars = document.createElement('div');
  btnGenerateCars.className = 'button generate-cars gray big';
  const GenerateCars = document.createElement('a');
...
const taskbarFragment = getTaskbarFragment();
...
const pageGarageInfoFragment = getGaragePageInfoFragment();
document
  .getElementsByClassName('garage-content')[0]
  .append(pageGarageInfoFragment);

function toPrevPageHandlerGarage() {
...
function toNextPageHandlerGarage() {
  const totalCars = document.getElementsByClassName('cars-in-garage')[0].innerHTML;
  const pageCount = Math.ceil(totalCars / carsNumberPerPageGarage);
...
async function renderWinnersInTable(sort, order) {
  const winnersRows = document.getElementsByClassName('tr-row');
  while (winnersRows.length > 0) {
    winnersRows[0].parentNode.removeChild(winnersRows[0]);
  }

  const winnersInTable = await ServerRequest.getWinners(
    currentPageWinners,
    carsNumberPerPageWinners,
    sort,
    order,
  );
  const winnersInTableArr = await winnersInTable.data;
  const pagesQuantity = Math.ceil(
    winnersInTable.totalCount / carsNumberPerPageWinners,
  );

  // Rendering amount of cars in the table of the WINNERS view
  document.getElementsByClassName('winners-amount')[0].innerHTML = winnersInTable.totalCount;
  ...
  const fragmentCarRowsInTable = document.createDocumentFragment();
  ...
  for (let index = 0; index < winnersInTableArr.length; index += 1) {
    ...
    const thCar = document.createElement('td');
    ...
    const thWins = document.createElement('td');
    thWins.className = 'th-Wins';
    thWins.innerHTML = `${elem.wins}`;
    tr.append(thWins);

    const thBestTime = document.createElement('td');
...
function toPrevPageHandlerWinners() {
...
function toNextPageHandlerWinners() {
  const totalWinners = document.getElementsByClassName('winners-amount')[0].innerHTML;
  const pageCount = Math.ceil(totalWinners / carsNumberPerPageWinners);
...
function getPaginationNavigation(isGarage) {
  ...
  const btnToNext = document.createElement('div');
...
const paginationNavigationGarage = getPaginationNavigation(true);
document
  .getElementsByClassName('garage-content')[0]
...
async function createNewCarHandler() {
  ...
  const newCarColor = formCreate.getElementsByClassName('input-color')[0].value;
...
async function updateSelectedCarHandler() {
  if (document.getElementsByClassName('selected').length) {
    const currentCarId = document
      ...
      .getAttribute('id');
    const currentInputUpdateNameElem = document.getElementsByClassName('input-text')[1];
    const currentInputUpdateColorElem = document.getElementsByClassName('input-color')[1];
    const currentCarName = currentInputUpdateNameElem.value;
    const currentCarColor = currentInputUpdateColorElem.value;
...
function getWinnersViewContent() {
  ...
  const divWinnersViewContent = document.createElement('div');
...
const winnersViewContent = getWinnersViewContent();
...
function getWinnersPageInfoFragment() {
  ...
  const divPageWinners = document.createElement('div');
...
const pageWinnersInfoFragment = getWinnersPageInfoFragment();
document
...
function getTableOfWinners() {
  ...
  const thWinsSpan = document.createElement('span');
  ...
  const thBestTimeSpan = document.createElement('span');
...
const paginationNavigationWinners = getPaginationNavigation(false);
document
  .getElementsByClassName('winners-content')[0]
...
const btnToGarage = document.getElementsByClassName('to-garage button')[0];
const btnToWinners = document.getElementsByClassName('to-winners button')[0];
